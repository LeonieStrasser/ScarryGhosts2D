...
public class Gast : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        guestState = behaviourState.arriving;                                                   // Anmerkung: Bis der Timer l�uft (erstes Mal myRoom erreicht wurde) ist der Gast im Checkin
    ...
    {
        iconRenderer.enabled = false; // F�r den Fall dass ein unhnappy Icon eingeschaltet war

        myRoom = newRoom;
        myCosts = myRoom.GetComponent<Room>().roomPrice;
        Waypoint targetOfMyRoom = newRoom.GetComponent<Room>().myWaypoint;

        myMovement.GoToNewTarget(targetOfMyRoom);                                               // Geht nur solange der Waypoint noch direkt auf dem myRoom Objekt liegr --- sind es sp�ter prefabs evtl �ndern.
    ...
    void GoAndWaitInLobby()
    {
        ...
        if (nextFreeWaitingPlace)
        {
            ...
            waitingPointIndex = waitIndex;                                                  // Der ArrayIndex des zugeordneten WaitingPoints wird gespecichert um diesen sp�ter wieder frei geben zu k�nnen. (im Selection Script)
        }
        else // Wenn die Wartepl�tze alle belegt sind
        {
    ...
    public void LeaveLobby(bool isNpcAngry)
    {
        if (waitingPointIndex >= 0)
        {
            ...
            Debug.LogWarning("Der Index des Waitingpoints wurde falsch in den NPC gespeichert. Er sollte negativ sein, wenn kein Platz mehr f�r den NPC in der Lobby war.");
    ...
    /// <summary>
    /// Wird aufgerufen wenn der Gast per Space in der Lobby ausgew�hlt wurde
    /// </summary>
